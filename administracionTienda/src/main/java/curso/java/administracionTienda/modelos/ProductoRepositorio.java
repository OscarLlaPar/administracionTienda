...
public interface ProductoRepositorio extends JpaRepository<Producto, Integer> {
	...
	@Query(value="SELECT * FROM productos p INNER JOIN detalles_pedido d ON p.id=d.id_producto INNER JOIN pedidos pd ON pd.id=d.id_pedido AND estado='E' GROUP BY id_producto ORDER BY SUM(unidades) DESC LIMIT 5", nativeQuery= true)
